...
<x-app-layout>
    <x-slot name="header">
        <div class="flex gap-x-4 items-center">
            <div class="font-semibold italic text-gray-700 text-lg">{{ __('Groups') }}</div>
            <Link href="#add-group" class="inline-flex items-center px-3 py-2 bg-indigo-500 hover:bg-indigo-600 text-white text-sm font-semibold rounded-md w-32 justify-center">
    ...
    <x-splade-modal name="add-group" max-width="sm">
        <x-splade-form :action="route('groups.store')">
    ...
    <div class="py-12">
        <div class="rounded border shadow bg-white p-5">
            <x-splade-table :for="$groups">
                ...
                <x-splade-cell edit>
                    <Link href="#edit-group-{{ $item->id }}" class="inline-flex items-center px-3 py-2 bg-indigo-500 hover:bg-indigo-600 text-white text-xs font-medium rounded-md">
                    ...
                    <x-splade-modal :name="'edit-group-'.$item->id" max-width="sm">
                        <x-splade-form :action="route('groups.update', $item)" method="PATCH" :default="$item">
                            <x-splade-input name="name" />
